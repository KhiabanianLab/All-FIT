...
for sam_count in range(1,samples+1,1):
	...
	mut_count -= 1
	mut_type[-1] += 1
	mut_high_ploidy = int(round(((sam_count-1)/100)*0.01*mut_count))
	for count in range(mut_high_ploidy):
		mutation = random.randint(6,7)
		mut_type[mutation] += 1
	mut_count -= mut_high_ploidy
	for count in range(mut_count):
		mutation = random.randint(0,5)
		mut_type[mutation] += 1
	mut_calc(mut_type,purity,out_sam)
	print sam_count, mut_type
