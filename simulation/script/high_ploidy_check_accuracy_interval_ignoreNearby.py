...
step = 25
#step = 10
highP = [[] for x in range(int(100/step))]	#[0-25),[25-50),[50-75),[75-]
highIndex = [[] for x in range(int(100/step))]
	...
	for k in range(0,2,1):
		...
		for each in sorted_xy_idx:
			...
			for ind in xy_index[y_idx][x_idx]:
				z[ind] = xy_count[y_idx][x_idx]*1000/len(highIndex[interval])
		idx = np.array(idx)
		z = np.array(z)
		...
		if k == 0:	
			...
			cax = ax.scatter(x, y, c=z, s=10, edgecolor='',cmap=cm, vmin=0, vmax=16)
		else:
			cax = ax.scatter(x, y, c=z, s=10, edgecolor='',cmap=cm, vmin=0, vmax=16)
		cb=fig.colorbar(cax)
...
xticklabel = ["[0.0-%.2f)\nN = %i"%(inter[0],len(highIndex[0]))]
for i in range(0,len(inter)-1,1):
	xticklabel.append("[%.2f-%.2f)\nN = %i"%(inter[i],inter[i+1],len(highIndex[i+1])))
